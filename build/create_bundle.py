...
class BundleCreator:
    ...
    def __init__(
        ...
        self.requirements_file: "Path|None" = Path(requirements_file) if requirements_file else None
        self.plist_template: "Path|None" = Path(plist_template) if plist_template else None
        self.desktop_template: "Path|None" = Path(desktop_template) if desktop_template else None
        self.apprun_template: "Path|None" = Path(apprun_template) if apprun_template else None
        self.windows_launcher_template: "Path|None" = Path(windows_launcher_template) if windows_launcher_template else None
    ...
    def create_bundle(self) -> bool:
        ...
        try:
            if self.current_platform == "windows":
                ...
                raise NotImplementedError(f"Platform not supported: {self.current_platform}")
    ...
    def _create_windows_launcher(self) -> Path:
        ...
        cpp_path = self.output_dir / self.platform_config['windows']['cpp_source']
    ...
    def _compile_windows_launcher(self, cpp_path: Path) -> Path:
        ...
        for compiler_args in compilers:
            ...
            try:
                ...
                print("Failed to compile launcher with: ", " ".join(compiler_args))
                continue
